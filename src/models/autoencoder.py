...
def autoencoder(seq_len=64, n_features=38):
    ...
    x = LSTM(32, return_sequences=True, unroll=True)(bottleneck)
...
def improved_cnn_lstm(seq_len=64, num_features=38, latent_dim=32):
    ...
    x = layers.LSTM(16, return_sequences=True, unroll=True)(x)
...
def improved_cnn_lstm_prob(seq_len=64, num_features=38, latent_dim=32):
    """
    Wrapper/alias for improved_cnn_lstm to satisfy imports in local training
    and Flower federated client configurations.
    """
    return improved_cnn_lstm(seq_len=seq_len, num_features=num_features, latent_dim=latent_dim)